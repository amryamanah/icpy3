...
class IC_Camera(object):
    ...
    def __init__(self, setting_file=None, show_dialog=False):
        self._handle = IC_GrabberDLL.create_grabber()

        if show_dialog:
            self._handle = IC_GrabberDLL.device_selection_dialog(self._handle)
        if setting_file:
            self._handle = IC_GrabberDLL.load_device_state(self._handle, c_char_p(setting_file.encode()))

        if not self._handle:
            raise IC_Exception(-1)
    ...
    def save_device_state(self, filename):
        """
        Save camera setting to filename
        :param filename string -- file name of the setting
        """
        err = IC_GrabberDLL.save_device_state(self._handle, c_char_p(filename.encode()))
        if err != 1:
            raise Exception(err)
    ...
    def remove_overlay(self, enable_overlay=False):
        """
        """
        err = IC_GrabberDLL.remove_overlay(self._handle, c_int(enable_overlay))
        if err != None:
            raise IC_Exception(err)
